      ...
      if (!isDevelopment()) {
        try {
          ...
          if (data && data.length > 0) {
            ...
            setSettings({
              general: { ...settings.general, ...groupedSettings.general },
              security: { ...settings.security, ...groupedSettings.security },
              notifications: { ...settings.notifications, ...groupedSettings.notifications },
              blockchain: { ...settings.blockchain, ...groupedSettings.blockchain },
            })
          }
        } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      if (isDevelopment()) {
        ...
        // Convert settings object to array of settings
        const settingsArray = []
        ...
        const { error } = await supabase.from("settings").upsert(settingsArray, { onConflict: "category,key" })
